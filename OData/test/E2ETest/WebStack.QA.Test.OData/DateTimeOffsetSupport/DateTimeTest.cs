...
namespace WebStack.QA.Test.OData.DateTimeOffsetSupport
{
    ...
    public class DateTimeTest
    {
        ...
        public async Task CreateFileTest(string mode, DateTimeOffset createTime)
        {
            var contentObject = new File() {
                FileId = 0,
                Name = "FileName",
                CreatedDate = createTime,
                ModifiedDates = new List<DateTimeOffset>()
            };

            await ResetDatasource();
            string requestUri = string.Format("{0}/{1}/Files", BaseAddress, mode);
            string content = JsonConvert.SerializeObject(contentObject, serializationSettings);
            ...
            Assert.Equal(result["FileId"], contentObject.FileId);
            Assert.Equal(result["Name"], contentObject.Name);
            Assert.Equal(result["CreatedDate"], contentObject.CreatedDate);
            Assert.Equal(result["ModifiedDates"].Count(), 0);
        }
        ...
        public async Task CreateDeleteFileEntityRoundTrip(string mode)
        {
            ...
            // POST ~/Files
            await CreateFileTest(mode, now);

            // GET ~/Files(6)
            HttpResponseMessage response = await Client.GetAsync(requestUri);
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Contains("\"CreatedDate\":\"" + now.ToString() + "\"", response.Content.ReadAsStringAsync().Result);
